...
void partb_server(char *address[]){
    ...
    if (brecv == -1) {
        ...
        if(!quiet_mode)debug_num("server", fm->protocol);
...
void partb_client(char *address[]){
    ...
    debug_num("fm->protocol", fm->protocol);
...
//TODO: check if s_ functions need address as an argument
//TODO: return 0 from all functions
//TODO: generate how much precent of the data arrived
//TODO: checksum with md5
...
long s_udp_ipv4_channel(){
    ...
    struct pollfd filedis[1];
    filedis[0].fd = sockfd;  
    filedis[0].events = POLLIN;
    struct timeval start_time, end_time;
    gettimeofday(&start_time, NULL);   
    ...
    while (1) {
        ...
        int res = poll(filedis, 1, -1);
        if (res == -1) {
            perror("poll");
            exit(1);
        }

        if (filedis[0].revents & POLLIN) {
            memset(data, 0, sizeof(data));
            addrlen = sizeof(struct sockaddr);
            error = recvfrom(sockfd, data, sizeof(data), 0, (struct sockaddr *)&cli, &addrlen);
            if (error == -1) {
                perror("recvfrom");
                exit(1);
            }
    ...
    if(quiet_mode == false){
        printf("received %d%% of the file\n", ((hundredMB/100)*rb_counter));
    }
...
//TODO: close sockets in all functions
//TODO: add partial recv print to all function- print in server
long s_uds_stream_channel(){
    ...
    int newfd = accept(sockfd, (struct sockaddr *)&cliaddr, &cli_addrlen);
    if(newfd == -1){
        perror("server side: accept");
        close(sockfd);
        close(newfd);
   ...
   if(quiet_mode==false) debug("socket accepted");

    //TODO: why use cliaddrlen twice? strange
    if(getpeername(newfd, (struct sockaddr *)&cliaddr, &cli_addrlen) == -1){
    ...
    while(rb_counter + chunk < hundredMB) {
            ...
            rb_counter += brecv;
            if(quiet_mode == false) printf("received %d/%d, total of %d/%d", brecv, chunk, rb_counter, hundredMB);
    }
    gettimeofday(&end_time, NULL);
...
long s_udp_ipv6_channel(){
    ...
    server_addr.sin6_port = htons(PARTB_PORT);
    server_addr.sin6_addr = in6addr_any;


     error = bind(sockfd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr_in6));
    if (error == -1) {
        perror("bind");
        exit(1);
    }
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            error = recvfrom(sockfd, data, sizeof(data), 0, (struct sockaddr *)&client_addr, &client_addr_len);
            if (error == -1) {
                perror("recvfrom");
                exit(1);
            }
...
long s_pipeline_channel(){
    ...
    while ((num_bytes_read = read(file, chunck, 1024)) > 0) {
        fwrite(chunck, 1, num_bytes_read, file_path);
    }
    ...
    if(quiet_mode==false)  printf("\nTime get to recv(pipe) :- %ld\n\n",time);
    return time;
}

//TODO: test with bigger chunks
...
long s_mmap_channel(){
 ...
 struct sockaddr_in server_addr, cli;
    ...
    server_addr.sin_port = htons(PARTB_PORT);
    ...
    while (1) {
        ...
        if (result == -1) {
            ...
            if (fds[0].revents & POLLIN) {
                ...
                int rcv_bytes = recvfrom(sock, buffer, 1024 - 1, 0, (struct sockaddr *)&cli, &clientlen);
                if (rcv_bytes < 0) {
                    perror("recvfrom_mmap");
...
//TODO: make sure i freed all buffers
...
long s_tcp_ipv6_channel(){
   ...
   if ((sockfd = socket(AF_INET6, SOCK_STREAM, 0)) == 0) {
    ...
    // Accept the data packet from client and verification
    int new_fd = accept(sockfd, (struct sockaddr*)&servaddr, &addrlen);
    if (new_fd < 0) {
        perror("server side: accept");
        exit(1);
    }
    //add_to_pfds(&s_pfds, new_fd, &fd_countt);
...
void c_tcp_ipv4_channel(char* address[]){
    ...
    if (send(sockfd, chunk100MB + sb_counter, sizeof(hundredMB-sb_counter), 0) < 0){
        perror("client side: send");
        exit(1);
    }
...
void c_tcp_ipv6_channel(char* address[]){
   struct sockaddr_in6 servaddr;
    ...
    char last_buf[hundredMB-sb_counter];
    memset(last_buf, '\0', sizeof(char)*(hundredMB-sb_counter));
    strncpy(last_buf, chunk100MB + sb_counter, hundredMB-sb_counter);
    if (send(sockfd, last_buf, sizeof(last_buf), 0) < 0){
        perror("client side: send");
        c_suicide_mode();
    }
    printf("sb_counter = %d/%d\n", sb_counter + (hundredMB-sb_counter), hundredMB);
    printf("completed\n");
    close(sockfd);
    ...
    while (1) {
        ...
        if (filedis[0].revents & POLLOUT) {
            if (fgets(data, sizeof(data), filepath) != NULL) {
                error = sendto(client_socket, data, strlen(data), 0, (struct sockaddr *)&servaddr, sizeof(struct sockaddr));
                if (error == -1) {
                    perror("sendto");
                    exit(1);
                }
...
void c_uds_stream_channel(char* address[]){
   ...
   debug("Sends done!");
    if(send(sockfd, chunk100MB + sb_counter, hundredMB - sb_counter, 0) == -1){
        perror("client side: send");
        close(sockfd);
        exit(1);
    }
   ...
   int sockfd;
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLOUT) {
            memset(data, 0, sizeof(data));
            if (fgets(data, sizeof(data), filepath) != NULL) {
                error = sendto(sockfd, data, strlen(data), 0, (struct sockaddr *)&servaddr, sizeof(struct sockaddr_in6));
                if (error == -1) {
                    perror("sendto");
                    exit(1);
                }
...
void c_pipeline_channel(char* address[]){
    ...
    if (fd == -1) {
        perror("open_pipe_client");
        exit(EXIT_FAILURE);
    }
    while ((bytes_read = fread(buffer, 1, 1024, file_path)) > 0) {
...
void c_mmap_channel(char* address[]){
    ...
    while (bytes_notSentYet > 0) {
        size_t chunk_size_DATA = (bytes_notSentYet > 1024) ? 1024 : bytes_notSentYet;
        bytes_SuccessSent = sendto(sockfd, fileDATA + offset, chunk_size_DATA, 0, (struct sockaddr *)&server_addr, sizeof(server_addr));
